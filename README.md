...
So I got vpinball working on Batocera *for a rpi4*!  I will say the performance is somewhat lacking but if you feel like trying it out, here it is.
...
I strongly recommend trying this from a SSD connected via a USB port.  Using a microsd card will be very slow.
...
But that may not work due to how the filesystems start up. So I added a ```/etc/init.d/S99swapstart```.  In it I put 2 lines
```
#!/bin/bash
/sbin/swapon -a
```
...
Batocera has some settings off the main menu to set the RPi to high performance mode. Do this.  Then go to a table, hold the "B" button and it'll bring up a menu.  In this menu tell it you have a "low power" computer.
